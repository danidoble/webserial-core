...
// Inject the BLE provider so AbstractSerialDevice uses Web Bluetooth
// instead of the native Web Serial API.
AbstractSerialDevice.setProvider(createBluetoothProvider());

/**
 * DemoBleDevice — Concrete implementation using the Web Bluetooth adapter.
 *
 * Connects to devices exposing the Nordic UART Service (NUS) over BLE GATT,
 * such as an nRF52, ESP32 BLE UART, or any module running a NUS-compatible
 * firmware.
 *
 * The `baudRate` parameter is carried for interface compatibility but has
 * no effect over BLE — the physical link speed is negotiated at the GATT level.
 */
export class DemoBleDevice extends AbstractSerialDevice<string> {
  constructor() {
    super({
      baudRate: 9600,
      dataBits: 8,
...
device.on("serial:need-permission", () => {
  log("Permission denied — select a valid BLE device.", "error");
  connectBtn.disabled = false;
});
...
if (!navigator.bluetooth) {
  ...
  log("Web Bluetooth ready. Click Connect to pair a BLE device.", "info");
}
