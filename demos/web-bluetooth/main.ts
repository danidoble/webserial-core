...
/**
 * demos/web-bluetooth/main.ts
 * Full-featured Web Bluetooth demo with chat console and code preview.
 */

import { AbstractSerialDevice, delimiter, raw } from "../../src/index";
import type { SerialDeviceOptions, SerialParser } from "../../src/types";
import { createBluetoothProvider } from "../../src/adapters/web-bluetooth/index";
import {
  initTheme,
  toggleTheme,
  addMessage,
...
// Inject the BLE provider once at module level.
...
function writeCfg(cfg: BleCfg): void {
...
function updatePreview(): void {
  ...
  previewTimer = setTimeout(() => {
    const cfg = readCfg();
    const rawName = (
      $<HTMLInputElement>("dl-name")?.value ?? "MyBleDevice"
    ).trim();
...
dlBtn?.addEventListener("click", () => {
  const cfg = readCfg();
  const rawName = (
    $<HTMLInputElement>("dl-name")?.value ?? "my-ble-device"
  ).trim();
  const className = sanitizeClassName(rawName);
  const isTS =
    (document.querySelector<HTMLInputElement>("input[name='dl-lang']:checked")
  ...
  if (dlType === "project") {
    downloadZip(
      [
        ...
        {
          name: "package.json",
          content: makePackageJson(rawName, isTS, "ble"),
        },
        {
          name: "index.html",
          content: makeIndexHtml(className, ext, "Web Bluetooth"),
        },
        {
          name: "README.md",
          content: makeReadme(
            ...
            ext,
            "Web Bluetooth",
            "Requires a Chromium browser. The device must expose a Nordic UART Service (NUS) via BLE GATT.",
          ),
        },
...
cfgImportInput?.addEventListener("change", () => {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (config.$version !== 1 || config.provider !== "ble") return;
...
connectBtn?.addEventListener("click", async () => {
  ...
  device = new ConfigurableSerialDevice(
    {
      ...
      autoReconnect: false, // BLE requires a user gesture to reconnect
  ...
  device.on("serial:connecting", () => {
    ...
    addMessage(messages, "Initiating Web Bluetooth connection…", {
      kind: "system",
    });
  });
  ...
  device.on("serial:connected", () => {
    ...
    enableInput(true);
    addMessage(messages, "Connected via Web Bluetooth!", { kind: "system" });
  });
  ...
  device.on("serial:need-permission", () => {
    ...
    addMessage(
      messages,
      "Permission denied — select a valid BLE device and allow access.",
      {
        kind: "error",
      },
    );
...
if (!navigator.bluetooth) {
  addMessage(
    messages,
    "Web Bluetooth is NOT supported in this browser or OS.",
    { kind: "error" },
  );
  connectBtn.disabled = true;
} else {
  addMessage(
    messages,
    "Web Bluetooth demo ready — configure settings and click Connect.",
    { kind: "system" },
  );
}
