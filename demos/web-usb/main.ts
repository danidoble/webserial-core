/**
 * demos/web-usb/main.ts
 * Full-featured WebUSB demo with chat console and code preview.
 */
...
function readCfg(): UsbCfg {
  ...
  return {
    usbControlInterfaceClass: num("cfg-ctrl-class", 255),
    usbTransferInterfaceClass: num("cfg-xfer-class", 255),
    baudRate: num("cfg-baud", 9600),
    dataBits: num("cfg-databits", 8) as 7 | 8,
    stopBits: num("cfg-stopbits", 1) as 1 | 2,
    parity: (sel("cfg-parity") || "none") as UsbCfg["parity"],
    flowControl: (sel("cfg-flow") || "none") as UsbCfg["flowControl"],
...
function writeCfg(cfg: UsbCfg): void {
  ...
  set("cfg-ctrl-class", cfg.usbControlInterfaceClass ?? 255);
  set("cfg-xfer-class", cfg.usbTransferInterfaceClass ?? 255);
...
function updatePreview(): void {
  ...
  previewTimer = setTimeout(() => {
    const cfg = readCfg();
    const rawName = (
      $<HTMLInputElement>("dl-name")?.value ?? "MyUsbDevice"
    ).trim();
...
dlBtn?.addEventListener("click", () => {
  const cfg = readCfg();
  const rawName = (
    $<HTMLInputElement>("dl-name")?.value ?? "my-usb-device"
  ).trim();
  const className = sanitizeClassName(rawName);
  const isTS =
    (document.querySelector<HTMLInputElement>("input[name='dl-lang']:checked")
  ...
  if (dlType === "project") {
    downloadZip(
      [
        ...
        {
          name: "package.json",
          content: makePackageJson(rawName, isTS, "usb"),
        },
        {
          name: "index.html",
          content: makeIndexHtml(className, ext, "WebUSB"),
        },
        {
          name: "README.md",
          content: makeReadme(
            ...
            ext,
            "WebUSB",
            "Requires a Chromium browser. Common USB-to-serial chips: CP2102 (0x10c4/0xea60), CH340 (0x1a86/0x7523).",
          ),
        },
...
cfgImportInput?.addEventListener("change", () => {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (config.$version !== 1 || config.provider !== "usb") return;
...
connectBtn?.addEventListener("click", async () => {
  ...
  // Re-inject provider with current UI values on each connect.
  ...
  device.on("serial:connecting", () => {
    ...
    addMessage(messages, "Connecting via WebUSB…", { kind: "system" });
  });

  device.on("serial:connected", () => {
    setStatus("connected", "Connected");
    enableInput(true);
    addMessage(messages, "Connected via WebUSB!", { kind: "system" });
  });
  ...
  device.on("serial:need-permission", () => {
    ...
    addMessage(
      messages,
      "Permission denied — allow access to the USB device.",
      {
        kind: "error",
      },
    );
  ...
  device.on("serial:reconnecting", () => {
    ...
    addMessage(messages, "Auto-reconnecting via WebUSB…", { kind: "system" });
  });
...
cmdAddBtn?.addEventListener("click", () => {
  ...
  savedCommands.push({
    ...
    name: n,
    value: v,
    mode: (cmdModeSelect?.value ?? "text") as "text" | "hex",
  });
...
lstAddBtn?.addEventListener("click", () => {
  ...
  dataListeners.push({
    ...
    name: n,
    pattern: p,
    match: (lstMatchSelect?.value ?? "exact") as DataListener["match"],
  });
...
addMessage(
  messages,
  "WebUSB demo ready — CP2102/ESP32 (0x10c4/0xea60), CH340/Arduino (0x1a86/0x7523). Configure and click Connect.",
  { kind: "system" },
);
